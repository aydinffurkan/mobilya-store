...
export default function ProductsFilter({
  ...
  const push = useCallback(
    (overrides: Record<string, string>) => {
      const base = { q, category: initialCategory, min, max, sort: initialSort }
      const merged = { ...base, ...overrides }
      const params = new URLSearchParams()
      Object.entries(merged).forEach(([k, v]) => { if (v) params.set(k, v) })
      const qs = params.toString()
      router.push(`/urunler${qs ? `?${qs}` : ''}`)
    },
    [q, initialCategory, min, max, initialSort, router]
  )

  const hasFilters = initialQ || initialCategory || initialMin || initialMax
  ...
  return (
    <div className="mb-8 space-y-4">
      ...
      <div className="flex flex-wrap gap-2 items-center">
        <button
          ...
          className={`px-4 py-1.5 rounded-full text-sm border transition-colors ${
            !initialCategory
              ...
              : 'border-border hover:bg-[#C8B8A6] hover:border-[#C8B8A6] hover:text-[#222222]'
          }`}
        >
          Tümü
        </button>
        {categories.map((cat) => (
          <button
            key={cat.slug}
            type="button"
            onClick={() => push({ category: initialCategory === cat.slug ? '' : cat.slug })}
            className={`px-4 py-1.5 rounded-full text-sm border transition-colors ${
        ...
        {hasFilters && (
          <button
            type="button"
            onClick={() => { setQ(''); setMin(''); setMax(''); router.push('/urunler') }}
            className="ml-auto flex items-center gap-1.5 px-3 py-1.5 text-sm text-muted-foreground hover:text-destructive transition-colors"
          >
      ...
      <p className="text-sm text-muted-foreground">
        ...
        {initialCategory && categories.find(c => c.slug === initialCategory) && (
          <span className="font-medium text-foreground"> — {categories.find(c => c.slug === initialCategory)!.name}</span>
        )}
      </p>
    </div>
  )
