...
export default function WhatsAppWidget({ phone, message }: Props) {
  const stickyBarVisible = useStickyBarStore((s) => s.visible)
  const href =
    'https://wa.me/' +
    phone.replace(/\D/g, '') +
  ...
  return (
    <a
      ...
      className={`fixed right-5 z-50 w-14 h-14 rounded-full shadow-xl flex items-center justify-center hover:scale-110 transition-all duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-[#25D366] ${
        stickyBarVisible ? 'bottom-24 sm:bottom-5' : 'bottom-5'
      }`}
