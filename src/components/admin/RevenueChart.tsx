  ...
  return (
    <div className="bg-white border border-border rounded-2xl p-5">
      ...
      {data.every((d) => d.value === 0) ? (
        ...
        <div className="flex items-end gap-1.5 h-44 min-w-[360px]">
          {data.map((d) => (
            <div key={d.label} className="flex-1 flex flex-col items-center gap-1.5 group relative">
              <div className="relative w-full flex items-end justify-center" style={{ height: '140px' }}>
                <div
                  className="w-full max-w-[28px] bg-[#222222] rounded-t-md transition-all group-hover:bg-[#222222]"
                  style={{ height: `${Math.max(2, (d.value / max) * 140)}px` }}
                />
                <div className="absolute -top-7 left-1/2 -translate-x-1/2 bg-[#222222] text-white text-[11px] px-2 py-1 rounded-md whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10">
